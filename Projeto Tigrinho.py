...
def atualizar_saldo(usuario, novo_saldo):#Alteração de informações dentro do arquivo txt referente ao saldo do /Login/#
    linhas = []
    with open("usuarios.txt", "r") as arquivo:
        for linha in arquivo:
            dados = linha.strip().split(",")
...
def config_banco(valor):#Definimos o Arquivo Txt para armazenar as informções de "Contas" definidas pelos usuarios#
    resul = False
...
def gerar_matriz(status):#Função Responsavel por Gerar as Matrizes#
  simbolos = ["💸", "🐅", "🐯"]
  ...
  K = 4 if influencer else 7#Definimos os status de "influencer" responavel pelos //niveis de vitoria definindo o status como influencer, fazendo referencia a VIDA REAL #
  if saldoAtual_bando <= 40000:
    K += 3 if influencer else 2

  if sorte > K:#Manipulação de Propabilidade, controlamos as variações de vitoria  detro do codigo #
...
def verificar_vitoria_spin(matriz): #Função Responsavel por determinar os parametros de vitoria#
  resul, quant = None, 0
  # diagonal
...
# jogo
def jogar(nome, saldo, status, param = "-n"): #função que ativa o jogo#
    def calc_valor():
    ...
    while saldo_aposta > saldo:
      saldo_aposta = float(input("Saldo insuficiente. Por favor, insira um valor coerente ao que tem em conta,ou recarregue o seu saldo"))
    while True:
        rodada += 1
        print(f"\n=== Giro {rodada} ===")
        ...
        if not verificar_resposta(resposta):
          ...
          print("Não há saldo em conta! Recarregue para continuar.")
          break
    ...
    if continuar == "s":
        ...
        print("Obrigado por jogar! Aguardamos o seu retorno! 🐯")
